...
class _FakeStock:
    ...
    through its story: quiet -> fakes to T1 one way (trapped) -> snaps back to the
    OPPOSITE box edge (reverse entry). This lets us watch the REVERSE state machine
    come alive on fake data exactly the way it will on real data.
    """
...
class FakeProvider(DataProvider):
    ...
    def __init__(self):
        self._stocks: dict[str, _FakeStock] = {
            # 360ONE — the real Excel row. Inside-day: H 1198, L 1170.2, X 27.8,
            # sell_t1 1156.3. REVERSE BUY:
            #   1190 quiet -> 1155 fakes DOWN to sell_t1 (trapped) -> 1199 snaps back
            #   to box high H 1198 (reverse BUY)
            "360ONE": _FakeStock(
                mon=OHLC(1179.8, 1188.0, 1165.2, 1170.6),
                tue=OHLC(1164.4, 1217.9, 1164.3, 1191.6),
                wed=OHLC(1198.0, 1198.0, 1170.2, 1190.0),
                price_script=[1190.0, 1155.0, 1199.0],
            ),
            # RELIANCE — inside-day: H 2995, L 2955, buy_t1 3015. REVERSE SELL:
            #   2970 quiet -> 3016 fakes UP to buy_t1 (trapped) -> 2955 snaps back to
            #   box low L 2955 (reverse SELL)
            "RELIANCE": _FakeStock(
                mon=OHLC(2950.0, 2990.0, 2900.0, 2960.0),
                tue=OHLC(2965.0, 3000.0, 2940.0, 2980.0),
                wed=OHLC(2975.0, 2995.0, 2955.0, 2970.0),
                price_script=[2970.0, 3016.0, 2955.0],
            ),
